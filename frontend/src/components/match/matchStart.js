...
export function matchStart(canvas) {
      ...
      if (this.y + gameField.gabsrit / 2 > gameField.y) {
        if (this.image) {
          ...
          ctx.drawImage(
            ...
            568,
            568,
            (this.x - (this.aWidth - this.width) / 2) * (mas + booMas) +
              offsetX,
            this.y * (mas + booMas) + offsetY,
            this.aWidth * mas,
            40 * mas
  ...
  class GameField {
    ...
    fieldCollision(cursor) {
      ...
      if (
        ...
        const i = Math.floor(
          (cursor.y - y) / (cellData.height * (mas + booMas))
        );
        const k = Math.floor(
          (cursor.x - x) / (cellData.width * (mas + booMas))
        );
          ...
          this.cellsArrey.forEach((controlCell) => {
            ...
            if (controlItem && controlItem !== item) {
              if (
                ...
                controlItem.y > 0 &&
                !(
                  Math.floor(item.x + item.width) <=
                    Math.floor(controlItem.x) ||
                  Math.floor(item.x) >=
                    Math.floor(controlItem.x + controlItem.width) ||
                  Math.floor(item.y) +
                    Math.floor(item.height <= controlItem.y) ||
                  Math.floor(item.y) >=
  ...
  getInterval(intervalData, removeEListenerMatch);
}
